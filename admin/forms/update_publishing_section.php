<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($_FILES["fileToUpload"]["size"]>0){
            ...
            $stmt = $conn->prepare('UPDATE homepage SET data2 = ?, data3 = ?, data4 = ?, data5 = ? WHERE tag = ?;');
            $stmt->bind_param("sssss", $data2, $data3, $data4, $data5, $tagName);
            if($stmt->execute()){
                $isSuccess = true;
            }
        }
